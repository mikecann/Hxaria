...
utils.RequestAnimationFrame.request = function(method) {
	$s.push("utils.RequestAnimationFrame::request");
	var $spos = $s.length;
	var requestAnimationFrame = window.requestAnimationFrame || (window.webkitRequestAnimationFrame || (window.mozRequestAnimationFrame || (window.oRequestAnimationFrame || (window.msRequestAnimationFrame || function(method1,element) {
		$s.push("utils.RequestAnimationFrame::request@20");
		var $spos = $s.length;
...
Shaders = function() { }
Shaders.__name__ = ["Shaders"];
Shaders.getVS = function() {
	$s.push("Shaders::getVS");
	var $spos = $s.length;
	$s.pop();
	return "\r\n\t\t\r\n\t\t\tuniform float amplitude;\r\n\t\t\tattribute float size;\r\n\t\t\tattribute vec3 customColor;\r\n\r\n\t\t\tvarying vec3 vColor;\r\n\r\n\t\t\tvoid main() {\r\n\r\n\t\t\t\tvColor = customColor;\r\n\r\n\t\t\t\tvec4 mvPosition = modelViewMatrix * vec4( position, 1.0 );\r\n\r\n\t\t\t\tgl_PointSize = size;\r\n\t\t\t\t//gl_PointSize = size * ( 300.0 / length( mvPosition.xyz ) );\r\n\r\n\t\t\t\tgl_Position = projectionMatrix * mvPosition;\r\n\r\n\t\t\t}\r\n\t\t\r\n\t\t";
	$s.pop();
}
Shaders.getFS = function() {
	$s.push("Shaders::getFS");
	var $spos = $s.length;
	$s.pop();
	return "\r\n\t\t\r\n\t\t\tuniform vec3 color;\r\n\t\t\tuniform sampler2D texture;\r\n\r\n\t\t\tvarying vec3 vColor;\r\n\r\n\t\t\tvoid main() {\r\n\r\n\t\t\t\tgl_FragColor = vec4( color * vColor, 1.0 );\r\n\t\t\t\tgl_FragColor = gl_FragColor * texture2D( texture, gl_PointCoord );\r\n\r\n\t\t\t}\r\n\t\t\r\n\t\t";
	$s.pop();
}
Shaders.prototype.__class__ = Shaders;
Std = function() { }
Std.__name__ = ["Std"];
Std["is"] = function(v,t) {
	$s.push("Std::is");
...
Game = function(p) {
	...
	$s.push("Game::new");
	var $spos = $s.length;
	this.width = js.Lib.window.innerWidth;
	this.height = js.Lib.window.innerHeight;
...
Game.__name__ = ["Game"];
Game.prototype.width = null;
Game.prototype.height = null;
Game.prototype.renderer = null;
Game.prototype.container = null;
Game.prototype.camera = null;
Game.prototype.scene = null;
Game.prototype.geometry = null;
Game.prototype.mouseX = null;
Game.prototype.mouseY = null;
Game.prototype.material = null;
Game.prototype.isMouseDown = null;
Game.prototype.lastMouseX = null;
Game.prototype.lastMouseY = null;
Game.prototype.init = function() {
	...
	this.camera.projectionMatrix = three.Matrix4.makeOrtho(js.Lib.window.innerWidth / -2,js.Lib.window.innerWidth / 2,js.Lib.window.innerHeight / 2,js.Lib.window.innerHeight / -2,-10,10);
	...
	while(_g < 100) {
		...
		while(_g1 < 100) {
			var xi = _g1++;
			var vector = new three.Vector3(xi * 17,yi * 17,0);
			this.geometry.vertices.push(new three.Vertex(vector));
		}
	}
	...
	var _g1 = 0, _g = vertices.length;
	while(_g1 < _g) {
		var i = _g1++;
		this.material.attributes.size.value[i] = 16;
		var c = new three.Color();
		c.r = Math.random();
		c.g = Math.random();
		c.b = Math.random();
		this.material.attributes.customColor.value[i] = c;
	}
	this.renderer = new three.WebGLRenderer({ clearAlpha : 1});
	this.renderer.setSize(this.width,this.height);
	this.renderer.sortObjects = false;
	this.renderer.setClearColorHex(11259375,1);
	this.container.appendChild(this.renderer.domElement);
	new js.JQuery(js.Lib.window).resize($closure(this,"onResize"));
	new js.JQuery(js.Lib.document).mousemove($closure(this,"onDocumentMouseMove"));
	new js.JQuery(js.Lib.document).mousedown(function(e) {
		$s.push("Game::init@121");
		var $spos = $s.length;
		me.isMouseDown = true;
		$s.pop();
	});
	new js.JQuery(js.Lib.document).mouseup(function(e) {
		$s.push("Game::init@122");
		var $spos = $s.length;
...
Game.prototype.getMaterial = function() {
	$s.push("Game::getMaterial");
	var $spos = $s.length;
	var attributes = { size : { type : "f", value : []}, customColor : { type : "c", value : []}};
	var uniforms = { amplitude : { type : "f", value : 1.0}, color : { type : "c", value : new three.Color(16777215)}, texture : { type : "t", value : 0, texture : three.ImageUtils.loadTexture("assets/terrain01.png")}};
	var $tmp = new three.MeshShaderMaterial({ uniforms : uniforms, attributes : attributes, vertexShader : Shaders.getVS(), fragmentShader : Shaders.getFS(), depthTest : false, transparent : true});
	$s.pop();
	return $tmp;
	$s.pop();
}
Game.prototype.onResize = function(e) {
	$s.push("Game::onResize");
	var $spos = $s.length;
	this.width = js.Lib.window.innerWidth;
	this.height = js.Lib.window.innerHeight;
	...
	this.renderer.setSize(this.width,this.height);
	haxe.Log.trace("canvas resize: " + this.width + ", " + this.height,{ fileName : "Game.hx", lineNumber : 163, className : "Game", methodName : "onResize"});
	$s.pop();
}
Game.prototype.onDocumentMouseMove = function(e) {
	$s.push("Game::onDocumentMouseMove");
	var $spos = $s.length;
	if(this.isMouseDown) {
		this.camera.translateX(this.lastMouseX - e.pageX);
		this.camera.translateY(e.pageY - this.lastMouseY);
	}
...
Game.prototype.render = function() {
	$s.push("Game::render");
	var $spos = $s.length;
	utils.RequestAnimationFrame.request($closure(this,"render"));
	var sizes = this.material.attributes.size.value;
	var colors = this.material.attributes.customColor.value;
	...
	this.renderer.render(this.scene,this.camera,null);
	$s.pop();
}
...
Main = function(p) {
	...
	$s.push("Main::new");
	var $spos = $s.length;
	js.Lib.window.onload = $closure(this,"onWindowLoad");
	$s.pop();
}
Main.__name__ = ["Main"];
Main.main = function() {
...
Main.prototype.onWindowLoad = function(e) {
	$s.push("Main::onWindowLoad");
	var $spos = $s.length;
	new Game();
	$s.pop();
}
